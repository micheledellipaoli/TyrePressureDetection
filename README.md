...
* **Interpreter**: a function which **retrieves** the **last 5 records** from Database and **interprets** the **data**:
    ...
    * if these 5 timestamps **fit** into a period of time of **1 minute** and the corresponding pressure values are **sorted** in **decreasing order**, then the function will send a message to the topic "iot/console" reporting: "**Warning, possible tyre puncture!**"; 
    * if these timestamps **don't fit** into a period of time of **1 minute**, or **only** the **last pressure value** is **lower** than the standard one, then the function will send a message to the topic "iot/console" reporting: "**Low tyre pressure registered!**";
    * if the **last pressure value** is **greater** or **equals** to the **standard one**, while the second-last pressure value is lower than the standard one, the function will send a message to topic "iot/console" reporting: "**Tyre pressure restored!**".
...
5. On the Nuclio Dashboard, click on the "Create project" button.
6. Fill the form to create a New Project, then click on the "Create" button.
7. Click on the Project just created to open it.

<br>

8. Click on the "Create Function" button to create a new function.
9. Click on the "Import" option above, and then on the "Import" button below.
10. Select the ***mqtt_consume_tyre_pressure*** YAML file to import the "Consume Tyre Pressure" function.
11. Click on the "Create Function" button.
12. Edit the "Consume Tyre Pressure" function by replacing the following String values with the parameters of the MySQL Instance you've just created in the previously step: 
    ````shell
    var DATABASE_HOST = "localDatabaseHost";
    var DATABASE_PORT = "3306";
    var DATABASE_USER = "localDatabaseUser";
    var DATABASE_PASSWORD = "localDatabasePassword"; 
    var DATABASE_NAME = "localDatabaseName"; 
    var DATABASE_TABLE_NAME = "localDatabaseTable";
    ````

13. Edit the "Consume Tyre Pressure" function by replacing the [localIpAddress] parameter with your local IP Address in the 18-th line of code:
    ```shell
    18       amqp.connect('amqp://guest:guest@[localIpAddress]:5672').then(function(conn) {
    ```
13. Go to the "Triggers" section above, click on the '>' symbol to open the "myMqttTrigger" trigger form, then edit the "URL" element by replacing the [localIpAddress] parameter with your local IP Address:
    ```shell
    URL
    guest:guest@[localIpAddress]:1883
    ```
13. Click the "Deploy" button to deploy the function.

<br>

8. Click on the "Create Function" button to create a new function.
9. Click on the "Import" option above, and then on the "Import" button below.
10. Select the ***mqtt_interpreter*** YAML file to import the "Interpreter" function.
11. Click on the "Create Function" button.
12. Edit the "Interpreter" function by replacing the following String values with the parameters of the MySQL Instance you've just created in the previously step: 
    ````shell
    var DATABASE_HOST = "localDatabaseHost";
    var DATABASE_PORT = "3306";
    var DATABASE_USER = "localDatabaseUser";
...
13. Edit the "Interpreter" function by replacing the [localIpAddress] parameter with your local IP Address in the 128-th line of code:
    ```shell
...
8. Click on the "Create Function" button to create a new function.
9. Click on the "Import" option above, and then on the "Import" button below.
10. Select the ***mqtt_simulate_low_tyre_pressure*** YAML file to import the "Simulate Low Tyre Pressure" function.
11. Click on the "Create Function" button.
12. Edit the "Simulate Low Tyre Pressure" function by replacing the [localIpAddress] parameter with your local IP Address in the 2nd line of code:
    ```shell
...
14. Click on the "Create Function" button to create a new function.
15. Click on the "Import" option above, and then on the "Import" button below.
16. Select the ***mqtt_simulate_tyre_puncture*** YAML file to import the "Simulate Tyre Puncture" function.
17. Click on the "Create Function" button.
18. Edit the "Simulate Tyre Puncture" function by replacing the [localIpAddress] parameter with your local IP Address in the 2nd line of code:
    ```shell
    2   var mqtt_url = url.parse(process.env.CLOUDAMQP_MQTT_URL || 'mqtt://guest:guest@[localIpAddress]:1883');
    ```

19. Click the "Deploy" button to deploy the function.

<br>

20. Click on the "Create Function" button to create a new function.
21. Click on the "Import" option above, and then on the "Import" button below.
22. Select the ***mqtt_restore_tyre_pressure*** YAML file to import the "Restore Tyre Pressure" function.
23. Click on the "Create Function" button.
24. Edit the "Restore Tyre Pressure" function by replacing the [localIpAddress] parameter with your local IP Address in the 2nd line of code:
    ```shell
    2   var mqtt_url = url.parse(process.env.CLOUDAMQP_MQTT_URL || 'mqtt://guest:guest@[localIpAddress]:1883');
    ```

25. Click the "Deploy" button to deploy the function.

<br>
<br>

#### 6) Run the system
Learn how to run the system by following the next steps. 
